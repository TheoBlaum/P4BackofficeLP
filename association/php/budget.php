...
require 'config.php'; // On inclut le fichier de configuration qui contient la connexion à la base de données

    if (!isset($_SESSION['user_id'])) {  // On vérifie si l'utilisateur est connecté en regardant s'il y a un user_id dans la session
    header("Location: login.php");  // L'utilisateur est redirigé vers la page de connexion s'il n'est pas connecté
    exit; // Arrête l'exécution du script après la redirection
}

$userId = $_SESSION['user_id']; // On récupère l'identifiant de l'utilisateur depuis la session
$query = $pdo->prepare("SELECT role FROM benevoles WHERE id = ?"); // On Prépare une requête pour récupérer le rôle de l'utilisateur connecté depuis la base de données
$query->execute([$userId]); // On exécute la requête en passant l'ID de l'utilisateur en paramètre
$user = $query->fetch(PDO::FETCH_ASSOC); // On récupère les résultats sous forme de tableau associatif
$userRole = $user ? $user['role'] : null; // On stocke le rôle de l'utilisateur, ou null s'il n'est pas trouvé


try { // Tentative d'exécution de la requête pour récupérer toutes les entrées de la table budget
    $stmt = $pdo->prepare("SELECT * FROM budget ORDER BY date DESC, heure DESC"); // On prépare une requête SQL pour sélectionner toutes les entrées de la table budget
    $stmt->execute(); // On exécute la requête
    
    
    $montant = $stmt->fetchAll(PDO::FETCH_ASSOC); // On récupère tous les résultats sous forme de tableau associatif
    } catch (PDOException $e) { // En cas d'erreur SQL, affiche un message d'erreur et arrête l'exécution du script
    die("Erreur lors de la récupération des données : " . $e->getMessage()); // Cela permet de gérer proprement les erreurs et ne pas afficher de page blanche
    }

$stmt2 = $pdo->query("SELECT SUM(montant) AS total FROM budget"); // On exécute une requête SQL pour obtenir la somme totale du montant du budget

$total = $stmt2->fetch(PDO::FETCH_ASSOC)['total']; // On récupère la valeur du total dans un tableau associatif
?>
